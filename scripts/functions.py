...
def preprocess_data(anime, anime_list):
    ...
    """
    anime_names = anime.select("MAL_ID", "Name", "Popularity").withColumnRenamed("MAL_ID", "anime_id")
    ...
    anime_list = anime_list.join(anime_names, anime_list.anime_id == anime_names.anime_id, "inner").drop(
        anime_names.anime_id)
    ...
    anime_list = anime_list.withColumn("anime_total_votes", count("*").over(Window.partitionBy("anime_id"))) \
        .withColumn("user_total_votes", count("*").over(Window.partitionBy("user_id")))
    anime_df_filtered = anime_list.filter((anime_list.anime_total_votes > 5000) & (anime_list.user_total_votes > 1500))
...
def create_matrix(anime_df_filtered):
    ...
    """
    indexer = [StringIndexer(inputCol=column, outputCol=column + "_index") for column in ["user_id", "anime_id"]]
...
def baseline(train_df):
    ...
    # find top-5 anime
    temp_arr = train_df.dropDuplicates(["anime_id_index"]).sort("Popularity").filter(train_df.Popularity > 0).limit(
        5).collect()

    most_pop = []
    for el in temp_arr:
        most_pop.append(el.anime_id_index)
...
def evaluate(name: str, pred, items_for_user_true):
    """
    ...
    print(f"{name} MAP at 5 = " + str(map_at_5))

    ndcg_at_5 = ndcg_evaluations.evaluate(
        items_for_user_all
    )

    print(f"{name} NDCG at 5 = " + str(ndcg_at_5))
...
def make_pred_first_model(model, train_df, transformed):
    ...
    dfs_pred_exclude_train = pred_df.alias("pred").join(
        ...
        (pred_df["user_id_index"] == train_df["user_id_index"]) & (
                pred_df["anime_id_index"] == train_df["anime_id_index"]),
        how='outer'
    )
...
def first_model(train_df, max_iter=15, reg_param=0.09, rank=10):
    ...
    """
    als = ALS(maxIter=max_iter, regParam=reg_param, rank=rank, userCol="user_id_index", itemCol="anime_id_index",
              ratingCol="rating",
...
def grid_search_first_model(train_df):
    ...
    """
    als_model = ALS(maxIter=15, userCol="user_id_index", itemCol="anime_id_index", ratingCol="rating",
                    coldStartStrategy="drop", nonnegative=True)
    params = ParamGridBuilder().addGrid(als_model.regParam, [0.001, 0.1, 1.0]).addGrid(als_model.rank,
                                                                                       [10, 15, 20]).build()
    evaluator = RegressionEvaluator(metricName='rmse', labelCol='rating', predictionCol='prediction')
    cv = CrossValidator(estimator=als_model, estimatorParamMaps=params, evaluator=evaluator, numFolds=4, parallelism=4)
    best_model = cv.fit(train_df)
    bm = best_model.bestModel

    print('Best Param (regParam): ', bm._java_obj.parent().getRegParam())
    print('Best Param (Rank): ', bm._java_obj.parent().getRank())
